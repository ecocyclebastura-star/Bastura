...
pub async fn download_and_save_image(
    ...
    let images_dir = app.path().app_data_dir().map_err(|e| {
        ...
        Ok(response) => {
            if response.status().is_success() {
                ...
                let mut file = fs::File::create(&tmp_file_path).map_err(|e| {
                    tracing::error!("Gagal membuat file gambar sementara {:?}: {}", tmp_file_path, e);
                    AppError::Unknown(format!("Gagal membuat file gambar: {}", e))
                })?;
                
                file.write_all(&bytes).map_err(|e| {
                    tracing::error!("Gagal menulis byte ke file gambar sementara {:?}: {}", tmp_file_path, e);
                ...
                fs::rename(&tmp_file_path, &file_path).map_err(|e| {
                    tracing::error!("Gagal me-rename file sementara {:?} ke {:?}: {}", tmp_file_path, file_path, e);
...
pub async fn read_image_as_base64(app: &tauri::AppHandle, filename: &str) -> Option<String> {
    let file_path = app.path().app_data_dir().ok()?.join("images").join(filename);
    ...
    match fs::read(&file_path) {
        Ok(bytes) => {
            ...
            let mime_type = if filename.to_lowercase().ends_with(".jpg") || filename.to_lowercase().ends_with(".jpeg") {
                "image/jpeg"
...
pub async fn cleanup_unused_images(app: &tauri::AppHandle, pool: &SqlitePool) {
    ...
    #[derive(sqlx::FromRow)]
    struct AnnRow { announcements_img: Option<String> }
    
    if let Ok(rows) = sqlx::query_as::<_, AnnRow>("SELECT announcements_img FROM announcements_cache").fetch_all(pool).await {
        for row in rows {
    ...
    #[derive(sqlx::FromRow)]
    struct EduRow { education_img: Option<String> }
    
    if let Ok(rows) = sqlx::query_as::<_, EduRow>("SELECT education_img FROM education_cache").fetch_all(pool).await {
        for row in rows {
                    ...
                    if let Some(filename) = entry.file_name().to_str() {
                        if !used_filenames.contains(filename) {
                            if let Err(e) = std::fs::remove_file(entry.path()) {
                                tracing::warn!("Gagal menghapus file gambar usang {:?}: {}", entry.path(), e);
                            } else {
                                tracing::info!("Berhasil menghapus file gambar usang: {}", filename);
                            }
                        }
                    }
                }
