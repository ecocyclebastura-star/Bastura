...
pub async fn run_smart_sync_service(state: &AppState) -> Result<bool, AppError> {
    ...
    let api_response = match res {
        Ok(response) => {
            if response.status().is_success() {
                ...
                tracing::warn!("API /updates merespons dengan status error: {}", response.status());
        ...
        Err(e) => {
            tracing::warn!("Gagal mengambil data /updates dari server (Mungkin offline): {}", e);
            return Err(e.into());
        }
    };
    ...
    if let Some(server_announcements_up) = &server_data.announcements_up {
        ...
        if needs_sync {
            ...
            if let Err(e) = crate::services::announcement_service::sync_announcements_from_server(state).await {
    ...
    if let Some(server_education_up) = &server_data.education_up {
        ...
        if needs_sync {
            ...
            if let Err(e) = crate::services::education_service::sync_education_from_server(state).await {
